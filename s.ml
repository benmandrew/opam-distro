module type S = sig
  type win10_release [@@deriving sexp]
  (** All Windows 10 release versions. *)

  type win10_ltsc [@@deriving sexp]
  (** All Windows Long-Term Service Branch releases. LTSC versions are aliased
      to the semi-annual release they're based on. *)

  type win_all [@@deriving sexp]
  (** All Windows 10/11 release versions and LTSC names. *)

  type win10_lcu [@@deriving sexp]
  type win10_revision [@@deriving sexp]

  type distro [@@deriving sexp]
  (** Supported Docker container distributions without aliases. *)

  type t [@@deriving sexp]
  (** Supported Docker container distributions including aliases. *)

  type os_family [@@deriving sexp]
  ...
  (** Latest Windows 10 Docker image available. May differ from
      {!win10_latest_release} if the Docker repository hasn't been updated. *)
  ...
  (** [builtin_ocaml_of_distro t] will return the OCaml version supplied with
      the distribution packaging, and [None] if there is no supported version. *)

  val human_readable_string_of_distro : t -> string
  (** [human_readable_string_of_distro t] returns a human readable version of
      the distribution tag, including version information. *)
  ...
  (** [package_manager t] returns the type of package manager used by that
      distribution. Many derived distributions (such as OracleLinux) share the
      same package manager from a base distribution (such as CentOS). *)
  ...
  (** [distro_of_tag s] parses [s] into a {!t} distribution, and [None]
      otherwise. *)
  ...
  (** [win10_base_tag base_image release] will return a tuple of Windows
      container base image and tag for which the base image of a Windows base
      image can be found (e.g. [mcr.microsoft.com/windows/servercore],[ltsc2022]
      which maps to [mcr.microsoft.com/windows/servercore:ltsc2022] on the
  ...
  (** [base_distro_tag ?arch t] will return a tuple of a Docker Hub
      ...
      found (e.g. [opensuse/leap],[15.0] which maps to [opensuse/leap:15.0] on
      the Docker Hub). This base image is in turn can be used to generate opam
  ...
  (** [win10_release_of_string] converts a Windows 10 version name as string to
      its internal representation. Ignores any KB number. *)
  ...
  (** [distro_arches ov t] returns the list of architectures that distribution
      [t] is supported on for OCaml compiler version [ov] *)
  ...
  (** [distro_supported_on arch ov distro] returns [true] if the combination of
      CPU [arch], compiler version [ov] is available on the distribution
      [distro]. *)

  (** {2 Opam build infrastructure support} *)

  type win10_release_status
  (** Windows 10 release status. *)

  val win10_release_status : win_all -> win10_release_status
  (** [win10_release_status v channel] returns the Microsoft support status of
      the specified Windows 10 release.
  ...
  (** [active_distros arch] returns the list of currently supported
      distributions in the opam build infrastructure. Distributions that are
      end-of-life upstream will rotate out of this list regularly. *)
  ...
  (** Tier 1 distributions are those supported for the full matrix of compiler
      ...
      {{:https://github.com/ocurrent/docker-base-images} Docker base images}
      will compile a base image for every OCaml version, so this list should be
      added to sparingly. *)

  val active_tier2_distros : Ocaml_version.arch -> t list
  (** Tier 2 distributions are those supported for a limited set of compiler
      versions in the opam build infrastructure. The distros in this list are
      also tested for packages in the opam repository. *)
